                ...
                if (err) {
                  ...
                  user
                    ...
                    .then((user) => {
                      ...
                      res.json({
                        success: true,
                        stats: "Password Changed Successfully",
                        profile: profile,
                      });
                    })
                    .catch((err = "Server Failed") => {
...
userRouter.get(
  ...
  (req, res, next) => {
    User.findById(req.user._id)
      .populate("subscribedLibraries._id")
      .then((user) => {
        if (user == null) {
          res.statusCode = 500;
          res.setHeader("Content-Type", "application/json");
      ...
      .then((user) => {
        if (user.favorites.id(req.body._id)) {
          ...
          res.json({
            success: false,
            status: "Process Failed",
            err: "This Item Already Exists In Your Favorites List",
          });
        } else {
          user.favorites.push({
            _id: req.body._id,
          ...
          user
            ...
            .then((user) => {
              ...
              res.json({
                success: true,
                status: "Item Added Successfully To Your Favorites List",
              });
            })
            .catch((err = "Server Failed") => {
              res.statusCode = 500;
      ...
      .then((user) => {
        ...
        if(itemFound == true){
          ...
          res.json({
            success: false,
            status: "Process Failed",
            err: "This Item Doesn't Exist In Your Favorites List",
          });
        }
      })
      .catch((err = "Server Failed") => {
